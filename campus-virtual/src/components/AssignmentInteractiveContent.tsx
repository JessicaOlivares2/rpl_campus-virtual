...
import PytestViewer from './PytestViewer'; // <-- Importamos el componente para mostrar fallidos
...
interface FeedbackState {
    ...
    details?: {
        testResults?: { name: string; passed: boolean; message: string; raw?: string }[];
        rawOutput?: string;
    };
}
...
export default function AssignmentInteractiveContent({
    ...
    const handleSubmitCode = async (e: React.FormEvent) => {
        ...
        const timeoutPromise: Promise<any> = new Promise((_, reject) =>
            setTimeout(() => reject(new Error("TimeoutError: Proceso de ejecución excedido.")), TIMEOUT_MS)
        );
        ...
        try {
            ...
            if (result.success) {
                ...
                setFeedback({
                    ...
                    details: { testResults: result.testResults, rawOutput: result.rawOutput }
                });

                if (result.passed) setIsCompleted(true);
            } else {
                setFeedback({
            ...
            setFeedback({
                success: false,
                message: errorMessage.includes("TimeoutError")
                    ? `🚨 Proceso Cancelado: Se excedió el límite de ${TIMEOUT_MS / 1000} segundos.`
                    : "Error de conexión o servidor interno.",
                details: { rawOutput: errorMessage }
            });
        }
    };
                                ...
                                <div className='w-full sm:w-2/3'>
                                    ...
                                    >
                                        <div className='flex items-center space-x-2'>
                                            ...
                                            {feedback.success && submissions[0]?.executionTimeMs !== null && (
                                        ...
                                        {/* PytestViewer solo muestra fallidos */}
                                        {feedback.details?.testResults && (
                                            <div className="mt-4">
                                                <PytestViewer results={feedback.details.testResults} />
                                            </div>
                                        )}
                                    </div>
                                </div>
